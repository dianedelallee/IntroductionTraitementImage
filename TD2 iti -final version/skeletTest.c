...
for (ichannel=0; ichannel<channel_number; ichannel++){
	...
	/*
	 * Affichage du tableau values afin de former l'histogramme.  values[i] contient le nombre d'occurrence du pixel de valeurs i dans l'image.
	 */
int ival = 0;
for(ival = 0;ival<256;ival++){
/*printf("nombre de pixels de valeur : %d = %d\n",ival,values[ival]);	
* affichage de l'histogramme
*/
	printf("pix%d - %d ", ival, values[ival]);
	int stars = 0;
	for (stars = 0; stars<(values[ival]/32); stars++){
		printf("*");
	}
	printf("\n");
}
