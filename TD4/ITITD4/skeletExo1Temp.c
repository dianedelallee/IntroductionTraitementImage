...
c=getchar();
switch (c){
case '1' :    // initialisation du tableau LUTIdentite
   for (identite =0; identite <256; identite++){
	LUT[identite] = identite;
   } break;
case '2' :    // initialisation du tableau LUTNegatif
   for (negatif = 0; negatif<256; negatif++){
	LUT[negatif] = 255-negatif;
    } break;
case '3' :   // initialisation du tableau seuillage a 128
   for ( seuillage = 0; seuillage <256; seuillage++){
	if (seuillage <= 128){  LUT[seuillage] = 0;  }
	else {  LUT[seuillage] = 255;  }
    } break;
case '4' :   // case 4 - initialisation du tableau de stretching lineaire (4points) pour a = 50 et pour b =205
   for(int stretchingFour =0; stretchingFour <256; stretchingFour ++ ){
	if(stretchingFour < 51){  LUT[stretchingFour] = 0;  }
	if(stretchingFour >204){  LUT[stretchingFour] = 255; }
	else{ LUT[stretchingFour] = 255*((float)(stretchingFour-51)/(float)(204-51)); }
   } break;
case '5' :  // case 5 - initialisation du tableau de stretching lineaire (2points) pour a = 50 et pour b =205
     for(int stretchingFour =0; stretchingFour <256; stretchingFour ++ ){
	if(stretchingFour < 50){  LUT[stretchingFour] = 0; }
	if(stretchingFour >205){  LUT[stretchingFour] = 255; }
	else{  LUT[stretchingFour] = 255*((float)(stretchingFour-50)/(float)(205-50)); }
   } break;
}// end switch
   ...
   for (ichannel=0; ichannel<channel_number; ichannel++) {
      for (ili=0; ili<nliin; ili++) {
		...
		processed_image[ichannel][ili*npxin+ipx] = LUT[origin_image[ichannel][ili*npxin+ipx]];
     	} /* Loop on pixels */
      } /* Loop on lines */
   } /* Loop on channels */
